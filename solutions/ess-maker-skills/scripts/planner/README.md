...
| `cli.py` | The command surface the skill invokes (`init`, `set-context`, `add-scenario`, `add-scenario-dependency`, `check-deps`, `add-task`, `assign`, `claim`, `set-state`, `capture-setup`, `mine`, `research`, `summary`, `validate`). |
