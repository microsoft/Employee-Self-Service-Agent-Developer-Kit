...
The checked-in script supports both a new environment and an environment where
authorization records already exist:

- create requests explicitly ask Dataverse to return the new record, so the
  delegated-authorization and team IDs are available to later steps;
- valid existing records and workflow shares are reused;
- every mutation is protected by PowerShell `ShouldProcess`, so `-WhatIf`
  performs no writes;
- multiple delegated authorizations or linked teams are rejected because
  Flow-RP requires an unambiguous authorization principal.

Do not modify or port the checked-in implementation for an individual
environment. Resolve unexpected duplicate or invalid records before rerunning
the script.
...
The script first validates a Dataverse token from Azure CLI. If that token is
unavailable or rejected, it uses the kit's normal Dataverse sign-in helper.
Do not pass an access token, password, client secret, or Workday credential to
the script.
...
On a new environment, the preview can also show that it would create the
delegated authorization and linked Access team. Confirm those records target
the intended bot and use team type `Access`.
