...
# Canonical denylist of ServiceNow tables that the LLM tools must never
# mutate. Both the client (typed CRUD methods) and the server (call_api
# escape hatch) import from here so the two layers cannot drift. Read
# (GET) is permitted because typed tools (resolve_user, etc.) need to
# query identity/security tables - mutation through the generic CRUD or
# table-API surface would let prompt injection delete admin users, modify
# security ACLs, or escalate privileges.
EXCLUDED_TABLES = frozenset({
    ...
    "sys_security_diag",
    "sys_properties",
    # scripting / web services (code-execution surface)
    "sys_script",
    "sys_script_include",
    "sys_script_action",
    "sys_script_client",
...
# Backwards-compat alias - kept so any older import keeps working until the
# next refactor.
_DENYLIST_TABLES = EXCLUDED_TABLES
...
def _validate_table(table: str, *, allow_denylist: bool = False) -> str:
    ...
    if not allow_denylist and table in EXCLUDED_TABLES:
        raise ValueError(
...
def _parse_retry_after(value: str) -> Optional[float]:
    ...
    ("Wed, 21 Oct 2026 07:28:00 GMT"). Returns seconds-from-now or None on
    parse failure.
    """
    if not value:
        return None
...
class ServiceNowClient:
    ...
    def __init__(self):
        ...
        # Long-lived AsyncClient so connection pool, keep-alive, and TLS
        # session are reused across the typical 5-50 calls in an LLM
        # session. Lazy-init in _ensure_client; closed in aclose().
    ...
    async def _ensure_client(self) -> httpx.AsyncClient:
        """Return the cached AsyncClient, creating it on first use."""
    ...
    async def aclose(self) -> None:
        """Close the cached AsyncClient. Call on shutdown."""
        if self._client is not None and not self._client.is_closed:
            await self._client.aclose()
        self._client = None

    async def request(self, method: str, path: str, **kwargs) -> dict:
        """Execute an HTTP request with retry on 429 and 502/503/504.

        Public surface: call_api in server.py routes through here; typed
        tools route through the higher-level methods below which also
        delegate here.
        """
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if resp.status_code == 429 or resp.status_code in (502, 503, 504):
                    parsed = _parse_retry_after(resp.headers.get("Retry-After", ""))
                    wait = parsed if parsed is not None else 2 ** attempt
                    # Add jitter to avoid thundering-herd retries.
                    wait += random.uniform(0, 1)
                    last_error = Exception(
                        f"Transient HTTP {resp.status_code}, "
                        f"Retry-After={resp.headers.get('Retry-After')!r}"
                    )
                    logger.warning(
                ...
                #
                # Note: previously this branch ALSO emitted the full body to
                # logger.debug. Removed in the round-1 review - even at DEBUG
                # level that's a PII leak path the comment above promised
                # was scrubbed. If an operator needs the body for triage,
                # they should reproduce the call with httpx logging enabled
                # explicitly via the mcp server's debug mode.
                try:
    ...
    # Backwards-compat alias for any in-tree caller still using _request.
    # New callers should use the public `request`.
    _request = request
    ...
    async def create_record(self, table: str, data: dict) -> dict:
        ...
        return await self.request("POST", f"/api/now/table/{table}", json=data)
    ...
    async def get_stats(self, table: str, query: str = "") -> dict:
        ...
        if query:
            params["sysparm_query"] = query
        return await self.request("GET", f"/api/now/stats/{table}", params=params)
