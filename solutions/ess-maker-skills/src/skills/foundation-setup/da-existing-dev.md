...
When candidates are returned, show their display names and ask the maker to choose one. Validate only the selected candidate through `validate-agent` or `attach`.
...
Present **Retry setup with another target** from `da-environment-target.md`, including its **Use an agent URL** choice. Directly inspect an exact agent selected through that URL.
