...
| INFRA-003 | External endpoint reachability from Power Platform egress | Critical | Enumerate external endpoints (Workday / ServiceNow / SAP SuccessFactors / custom HTTP) from the agent's connection config. Default path: read-only local TCP/TLS probe (shares INFRA-002's `probe_endpoint()`), bucketed by status. Live path (opt-in `--live-probe`, consent-gated): a transient cloud flow (Dataverse `workflow` row) probes from Power Platform's own egress, then is deleted — the kit's only mutating path, with guaranteed cleanup and orphan sweep. Read-only and idempotent by default | All enumerated endpoints reachable | Any endpoint unreachable (DNS failure or connection refused) — names the endpoint URL and the blocking hop | Timeout, TLS failure, or unverifiable endpoint; also emitted when the live egress probe is requested but unavailable and the local probe was used |
