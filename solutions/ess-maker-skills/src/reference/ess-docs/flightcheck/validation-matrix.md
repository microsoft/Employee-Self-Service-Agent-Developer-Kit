...
| SN-RUN-001 | ServiceNow active connector runtime health. Active, consent-gated probe: a transient cloud flow invokes one **read-only** ServiceNow operation (default `GetRecords` on the core `sys_user` table with `sysparm_limit=1`; allowlist-enforced) through the operator-owned service-account managed connection from Power Platform egress, then is deleted — the same mutating tenant path as `WD-RUN-001` (see INFRA-003), with guaranteed cleanup and orphan sweep. PASS when ServiceNow returns data; FAILED for an attributable connector failure (endpoint 404/405, throttle 429, server 500, a network/DNS/TLS/DLP block, or a connector-connection credential rejection 401/403). An HTTP 400 / BadRequest is treated as **indeterminate** (it reached ServiceNow but a wrong endpoint/table/operation and a ServiceNow business/validation fault are indistinguishable — and ServiceNow table/row ACL denial also surfaces as 400, not 403) and degrades to the passive run-history signal rather than FAILING the connection. When the probe is not run (flag omitted/declined, an OAuth-invoker-only install, the operator owns no eligible connection, or a bad operation override) it falls back to recent ServiceNow connector run history; a true pre-deployment state (no ServiceNow managed-connector connection) returns NOT_CONFIGURED. | High | Consent-gated transient connector probe + passive run-history fallback | [servicenow](https://learn.microsoft.com/en-us/copilot/microsoft-365/employee-self-service/servicenow) |
