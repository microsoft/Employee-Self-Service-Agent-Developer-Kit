...
class TestNonInteractiveStdinDegradesToSkip:
    ...
    subprocess or a test/automation harness), where input()/getpass() raise
    EOFError immediately. That used to crash the whole Workday category with
    an unhandled EOFError (a WOR-ERR row that aborted every later Workday
    check). Both runtime prompts must instead degrade to an empty value so the
    caller records a clean SKIP.
    """
