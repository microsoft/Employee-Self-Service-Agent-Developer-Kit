...
def test_global_and_command_gates_require_canonical_da_completion() -> None:
    ...
    assert "If the user typed `/connect`" in instructions
    assert 'steps.SETUP-07.state: "done"' in instructions
    assert "even when runtime `connect_ready` is false" in instructions
