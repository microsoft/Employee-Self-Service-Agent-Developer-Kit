...
| **AgentBuilder Minimal Bot API** (`/copilotstudio/minimalBots/...`) | `validated` | Cassette at `agentbuilder_readiness.yaml` (recorder `tests/captures/record_agentbuilder_readiness.py`). | Internal Copilot Studio API. FlightCheck uses only direct agent lookup, explicit Dev-realm configuration, and the read-only component-fetch operation. |
| **Power Platform Connectivity API** (`api.{ring}.powerplatform.com/connectivity/...`) | `validated` | Cassettes at `agentbuilder_readiness.yaml` and `connectivity_connections.yaml`; recorder `tests/captures/record_agentbuilder_readiness.py`. | Environment-scoped physical connection inventory used by native DA readiness. The populated connection shape was captured from a live Preprod connection-settings read and sanitized to the fields consumed by FlightCheck. |
...
| **Power Platform Environment Management API** (`GET api.powerplatform.com/environmentmanagement/environments`) | `documented` | MS Learn `https://learn.microsoft.com/en-us/rest/api/power-platform/environmentmanagement/environments/list-environments-for-user` (api-version 2024-10-01). Consumed `EnvironmentList` and `EnvironmentResponse` field tables are copied into `tests/mocks/powerplatform.py`. | Preview user-scoped environment listing used only to resolve a maker-entered Dataverse URL without enumerating admin-only environments. No public OpenAPI/no-auth metadata is published, so `validatable` does not apply. |
| **Power Platform App Management API** (`GET/POST api.powerplatform.com/appmanagement/environments/{id}/applicationPackages...`) | `documented` | MS Learn `https://learn.microsoft.com/en-us/rest/api/power-platform/appmanagement/applications/get-environment-application-package` + `https://learn.microsoft.com/en-us/rest/api/power-platform/appmanagement/applications/install-application-package` (api-version 2024-10-01). Consumed model field tables and mock builders are in `tests/mocks/powerplatform.py`. | The install POST is an intentional durable setup write, not a disposable FlightCheck probe. Safety is fail-closed and idempotent by package identity: setup first reads the selected package state, skips POST for `Installed`/`TemplateInstalled`, never re-POSTs for in-progress states, persists the selected catalog product, and uses no automatic POST retries. Completion polling rereads the documented package collection; no undocumented operation-status endpoint is used. |
...
| **Power Automate flow lifecycle** (Dataverse `{env}/api/data/v9.2/workflows` create/activate/find/delete + `api.flow.microsoft.com/.../flows/{workflowid}/triggers/{trigger}/listCallbackUrl` + SAS-signed trigger invoke) | `validated` | Captured cassette `flightcheck_infra003_flow.yaml` (recorder `tests/captures/record_infra003_flow.py`). Create/activate/delete are **Dataverse Web API** ops (a cloud flow is a `workflow` row, category 5, per `https://learn.microsoft.com/en-us/power-automate/manage-flows-with-code`) — NOT `api.flow.microsoft.com` PUT-by-name (that 404s). Only the trigger callback URL comes from the Power Automate API (`Microsoft.ProcessSimple`, `api-version=2016-11-01`, audience `service.flow.microsoft.com//.default`); the invoke POSTs the runtime host returned by `listCallbackUrl`. | Backs INFRA-003 `--runtime-reachability` (production: `flightcheck/live_egress_probe.py`, mock: `tests/mocks/power_automate.py`). MUTATING API — the only FlightCheck path that writes to the tenant. Captured in a disposable environment; the transient probe flow is always deleted (delete 204) and AC7 idempotency is preserved via a deterministic display name (`flightcheck-infra003-probe`) + `$filter` orphan sweep. The `listCallbackUrl` response and the trigger runtime URL are SAS-signed secrets: the redactor strips `sig`/`sv`/`sp` and the dashless split env-id runtime hostname. |
| **Power Automate connector-bound flow lifecycle** (WD-RUN-001 v2, connector-bound Option 1) | `validated` | Captured cassette `flightcheck_wd_connector_probe.yaml` (recorder `tests/captures/record_flightcheck_wd_connector_probe.py`). Same Dataverse workflow lifecycle as INFRA-003, but `clientdata.properties.connectionReferences.<key>` binds the maker's real Workday connection id GUID with `source: "Embedded"`, plus one `OpenApiConnection` action on `shared_workdaysoap`. | Backs WD-RUN-001 v2 active Workday connector probe. MUTATING API behind the same explicit runtime-reachability consent gate. The capture proved create 201, activate 204, listCallbackUrl 200, invoke 200, delete 204 for read-only `GetWorkerMe`. It proves RaaS / REST managed-connector path fidelity only, not the plugin-tunneled generic-SOAP path. |
| **Power Automate connector-bound flow lifecycle** (SN-RUN-001 AC13 active probe) | `validated` | Captured cassette `flightcheck_sn_connector_probe.yaml` (recorder `tests/captures/record_flightcheck_sn_connector_probe.py`). Same Dataverse workflow lifecycle as the WD connector probe, but `clientdata.properties.connectionReferences.<key>` binds the maker's real ServiceNow connection id GUID with `source: "Embedded"`, plus one `OpenApiConnection` action on `shared_service-now` (operation `GetRecords`). | Backs SN-RUN-001 active ServiceNow connector probe. MUTATING API behind the same runtime-reachability consent gate. Captured live in PROD (2026-08-11) against an operator-owned connection: create 201, activate 200, invoke → connector 200 / OK for read-only `GetRecords` (`tableType=sys_user, sysparm_limit=1`), delete 204, no orphan. It proves the ServiceNow REST managed-connector path only. Note: activation succeeds only when the /flightcheck operator owns the bound connection; a connection owned by another maker returns activate 400 `ConnectionAuthorizationFailed` and the probe falls back to passive. **Deeper fault-capture pass (PROD 2026-08-12, cassette `flightcheck_sn_connector_probe_faults.yaml`, recorder env `ESS_SN_PROBE_SHAPES_JSON`):** the connector surfaces distinct HTTP statuses, it does NOT collapse everything into 400. Live shapes: valid read → 200/OK; invalid table → 400/BadRequest; restricted table (`sys_user_password`) → 400/BadRequest (NOT 403 — ACL denial collapses into 400); `GetRecord` with a non-existent sys_id → 404/NotFound; `GetRecords` with empty required params → fails at flow **activate** (before the connector runs), not a connector 400. The 401/403, 429, 500, and no-response (DNS/TLS/DLP) branches remain documented assumptions — they cannot be induced non-destructively against a healthy tenant. |
...
| Dataverse (flow lifecycle) | `POST {env}/api/data/v9.2/workflows` (create cloud flow — category 5, `Prefer: return=representation` → `workflowid` in body + `OData-EntityId` header) | 201 | `flightcheck_infra003_flow.yaml` |
| Dataverse (flow lifecycle) | `PATCH {env}/api/data/v9.2/workflows({id})` (activate — `{"statecode":1}`, `If-Match: *`) | 204 | `flightcheck_infra003_flow.yaml` |
| Dataverse (flow lifecycle) | `GET {env}/api/data/v9.2/workflows?$select=workflowid,name,statecode&$filter=name eq '...' and category eq 5` (orphan detection for AC7 cleanup) | 200 | `flightcheck_infra003_flow.yaml` |
| Dataverse (flow lifecycle) | `DELETE {env}/api/data/v9.2/workflows({id})` (delete transient probe flow) | 204 | `flightcheck_infra003_flow.yaml` |
| Power Automate (trigger) | `POST https://api.flow.microsoft.com/providers/Microsoft.ProcessSimple/environments/{env_id}/flows/{workflow_id}/triggers/manual/listCallbackUrl?api-version=2016-11-01` (SAS callback URL at `response.value`) | 200 | `flightcheck_infra003_flow.yaml` |
| Power Automate (trigger invoke) | `POST <SAS callback URL>` (`Prefer: wait` → synchronous body `{"reachableStatusCode": <int\|null>, "actionStatus": <str>}`; int = egress reached, null = blocked) | 200 | `flightcheck_infra003_flow.yaml` |
| Dataverse (flow lifecycle — **AC9 SPIKE Option 1, connector-bound GO**) | `POST {env}/api/data/v9.2/workflows` — create with `connectionReferences.<key> = {connectionName: <connection id GUID>, source: "Embedded", id: <apiId>, tier: "Integrated"}` (bind by the REAL connection id, NOT `connectionReferenceLogicalName`) + one `OpenApiConnection` action bound to `shared_workdaysoap`. | 201 | `flightcheck_wd_connector_probe.yaml` |
| Dataverse (flow lifecycle — **AC9 SPIKE Option 1, connector-bound GO**) | `PATCH {env}/api/data/v9.2/workflows({id})` activate (`{"statecode":1}`). **ACCEPTED**: `source: "Embedded"` binds the maker's connection at activation, fixing the logical-name `FlowMissingConnection` NO-GO. | 204 | `flightcheck_wd_connector_probe.yaml` |
| Power Automate (trigger — **AC9 SPIKE Option 1, connector-bound GO**) | `POST .../flows/{workflow_id}/triggers/manual/listCallbackUrl` on the activated flow → SAS callback URL at `response.value`. | 200 | `flightcheck_wd_connector_probe.yaml` |
| Power Automate (trigger invoke — **AC9 SPIKE Option 1, connector-bound GO**) | `POST <SAS callback URL>` (`Prefer: wait`) runs read-only `GetWorkerMe` over the real `shared_workdaysoap` managed-connector path → body `{workdayActionStatus:"Succeeded", workdayStatusCode:200}`; run `Succeeded`, action `Probe_Workday` `status=Succeeded code=OK`. | 200 | `flightcheck_wd_connector_probe.yaml` |
| Dataverse (flow lifecycle — **SN-RUN-001 AC13, connector-bound GO**) | `POST {env}/api/data/v9.2/workflows` — create with `connectionReferences.<key> = {connectionName: <ServiceNow connection id GUID>, source: "Embedded", id: <apiId>, tier: "Integrated"}` + one `OpenApiConnection` action bound to `shared_service-now` (operation `GetRecords`). | 201 | `flightcheck_sn_connector_probe.yaml` |
| Dataverse (flow lifecycle — **SN-RUN-001 AC13, connector-bound GO**) | `PATCH {env}/api/data/v9.2/workflows({id})` activate (`{"statecode":1}`). Succeeds only when the operator OWNS the bound connection; a non-owned connection returns **400 `ConnectionAuthorizationFailed`** and the probe falls back to passive. | 200 | `flightcheck_sn_connector_probe.yaml` |
| Power Automate (trigger — **SN-RUN-001 AC13, connector-bound GO**) | `POST .../flows/{workflow_id}/triggers/manual/listCallbackUrl` on the activated flow → SAS callback URL at `response.value`. | 200 | `flightcheck_sn_connector_probe.yaml` |
| Power Automate (trigger invoke — **SN-RUN-001 AC13, connector-bound GO**) | `POST <SAS callback URL>` (`Prefer: wait`) runs read-only `GetRecords` (`tableType=sys_user, sysparm_limit=1`) over the real `shared_service-now` managed-connector path → body `{connectorActionStatus:"Succeeded", connectorStatusCode:200, connectorErrorMessage:null, connectorErrorCode:"OK"}`; run `Succeeded`, action `Probe_ServiceNow` `status=Succeeded code=OK`. An invalid table returns `connectorStatusCode:400 / BadRequest`. | 200 | `flightcheck_sn_connector_probe.yaml` |
| Power Automate (trigger invoke — **SN-RUN-001 deeper fault-capture**) | `GetRecords` invalid table → `connectorStatusCode:400 / BadRequest`; `GetRecords` restricted table `sys_user_password` → `connectorStatusCode:400 / BadRequest` (ACL denial does NOT surface as 403); `GetRecord` with non-existent sys_id → `connectorStatusCode:404 / NotFound`. `connectorErrorMessage` is `null` on all (human faultstring lives behind the SAS outputsLink). | 200 (flow Response wraps the connector status) | `flightcheck_sn_connector_probe_faults.yaml` |
| Dataverse (flow activate — **SN-RUN-001 deeper fault-capture**) | `GetRecords` with empty required params fails at flow **activate** (the required connector parameter is missing from the flow definition), before any ServiceNow call — probe returns stage=activate, no connector status. | 400 (activate) | `flightcheck_sn_connector_probe_faults.yaml` |
...
| AgentBuilder Minimal Bot | `GET /copilotstudio/minimalBots/api/{agent_id}` (direct exact-agent lookup) | 200 | `agentbuilder_readiness.yaml` |
| AgentBuilder Minimal Bot | `GET /copilotstudio/minimalBots/alm/{agent_id}/configure?realm=0` (explicit Dev-realm configuration and ALM-family identity) | 200 | `agentbuilder_readiness.yaml` |
| AgentBuilder Minimal Bot | `POST /copilotstudio/minimalBots/api/{agent_id}/components` with `{}` (read-only authored component and logical connection-reference snapshot) | 200 | `agentbuilder_readiness.yaml` |
| Power Platform Connectivity | `GET https://api.{ring}.powerplatform.com/connectivity/environments/{environment_id}/connections` (environment physical connection inventory; `properties.apiId` + `properties.statuses`) | 200 | `agentbuilder_readiness.yaml`, `connectivity_connections.yaml` |
