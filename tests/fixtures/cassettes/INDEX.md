...
| **Dataverse Web API v9.2** | `documented` | MS Learn `https://learn.microsoft.com/power-apps/developer/data-platform/webapi/`. Per-org `$metadata` exists at `{org}/api/data/v9.2/$metadata` but requires auth, so it's not a no-tenant validation path. | Excellent prose docs with example responses for every operation. INFRA-002's consent-gated live probe uses the `workflow` entity operations here — create (`POST /workflows`), activate/deactivate (`PATCH /workflows({id})` `statecode`/`statuscode`), delete (`DELETE /workflows({id})`) — all documented; see MS Learn "Create and update workflows" / the `workflow` EntityType. This is the ONLY FlightCheck path that mutates Dataverse, and only under `--live-network-probe` consent (net-zero: the temporary flow is always deleted). |
| **Power Automate manual-trigger callback URL** (`/Microsoft.ProcessSimple/environments/{env}/flows/{flow}/triggers/manual/listCallbackUrl`) | `validated` | Hosted on `api.flow.microsoft.com`, `service.flow.microsoft.com//.default` token, **POST** (runtime/owner scope, NOT `/scopes/admin`). Returns `{"value": "<SAS-signed invoke URL>"}`. Capture pending — recorder `tests/captures/record_flightcheck_infra002.py` produces `flightcheck_infra002.yaml`. | Backs INFRA-002 live probe (`PowerPlatformAdminClient.list_callback_url`). Only meaningful for an ACTIVATED flow whose trigger is named `manual`. Verified live against a real tenant in the INFRA-002 research spike. |
| **Logic Apps manual-trigger callback invoke** (`POST {callbackUrl}` on `*.logic.azure.com` / regional Flow gateway) | `validated` | The SAS-signed URL returned by listCallbackUrl. The 2xx **response body is authored by the flow itself** (INFRA-002's `Response` action — see `flow_probe.build_client_data`), so the reachability contract (`reachable`, `http_status`, `redirect_location`, ...) is owned by FlightCheck, not the platform; the external contract is the invoke's HTTP status + redirect behaviour (302→re-POST, 202→poll). Capture pending — same recorder / `flightcheck_infra002.yaml`. | Backs INFRA-002 live probe (`flow_probe.trigger_and_read`). Verified live in the research spike (302 login redirect = reachable; `/invalid-url` redirect = invalid instance). |
...
| Power Automate (trigger callback) | `POST https://api.flow.microsoft.com/providers/Microsoft.ProcessSimple/environments/{env_id}/flows/{flow_id}/triggers/manual/listCallbackUrl` (INFRA-002 — returns `{"value": "<invoke URL>"}` for an activated flow's `manual` trigger; runtime/owner scope) | 200 | `flightcheck_infra002.yaml` _(capture pending)_ |
| Logic Apps (trigger invoke) | `POST {callbackUrl}` (INFRA-002 — invokes the temporary probe flow; 2xx body is the flow-authored reachability result, or 302 redirect handled by re-POST) | 200 / 302 | `flightcheck_infra002.yaml` _(capture pending)_ |
