...
| **Power Automate flow lifecycle** (`api.flow.microsoft.com/providers/Microsoft.ProcessSimple/environments/{env}/flows/{flow}` create/delete + `.../triggers/{trigger}/listCallbackUrl`) | `validated` | Captured cassette (**MUTATING capture — pending**). Operation set confirmed against the Power Automate Management connector reference `https://learn.microsoft.com/en-us/connectors/flowmanagement/` (Create Flow / Delete Flow / Turn On Flow / List Callback URL). Raw REST host `api.flow.microsoft.com`, provider `Microsoft.ProcessSimple`, `api-version=2016-11-01`, audience `service.flow.microsoft.com//.default` (same as the admin flow-listing rows). | Backs INFRA-003 `--live-probe`. **Tier decided = validated, but no cassette is captured yet, so the confirmed-cassette rows below are not present and the INFRA-003 live path stays DEFERRED** until the cassette lands (per AGENTS.md step 2: validated tier + no confirmed row ⇒ STOP). MUTATING API — the capture creates AND deletes a transient probe flow, so run it in a disposable environment; capture the delete response and the orphan-cleanup 404 for the AC7 idempotency test. The `listCallbackUrl` response and the triggered run URL are SAS-signed secrets: the redactor MUST strip `sig`/`sv`/`sp` query params. Trigger POST targets the runtime host returned by `listCallbackUrl`, not `api.flow.microsoft.com`. Capture wrapper: `tests/captures/record_infra003_flow.py`. |
