...
| **Power Automate connector-bound flow lifecycle** (WD-RUN-001 v2 AC9 spike Option 1) | `validated` | Captured cassette `flightcheck_wd_connector_probe.yaml` (recorder `tests/captures/record_flightcheck_wd_connector_probe.py` on spike branch `spike/wd-run-001-v2-ac9`). Same Dataverse workflow lifecycle as INFRA-003, but `clientdata.properties.connectionReferences.<key>` binds the maker's real Workday connection id GUID with `source: "Embedded"`, plus one `OpenApiConnection` action on `shared_workdaysoap`. | Backs WD-RUN-001 v2 active Workday connector probe. MUTATING API behind the same explicit runtime-reachability consent gate. The spike proved create 201, activate 204, listCallbackUrl 200, invoke 200, delete 204 for read-only `GetWorkerMe`. It proves RaaS / REST managed-connector path fidelity only, not the plugin-tunneled generic-SOAP path. |
