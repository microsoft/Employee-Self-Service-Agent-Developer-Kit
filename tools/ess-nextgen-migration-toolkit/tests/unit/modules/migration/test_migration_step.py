...
def test_mode_gated_step_runs_only_in_declared_mode() -> None:
    ...
    discover_ctx = MigrationContext(ExecutionMode="READONLY")
    ...
    migrate_ctx = MigrationContext(ExecutionMode="WRITEBACK")
...
def test_subclass_can_override_can_execute_with_super() -> None:
    ...
    ctx = MigrationContext(ExecutionMode="WRITEBACK")
    result = pipeline.run(ctx)
    assert len(result.Logs) == 0

    # Right mode AND has changes — runs
    ctx = MigrationContext(ExecutionMode="WRITEBACK", Changes=[None])  # type: ignore[list-item]
    result = pipeline.run(ctx)
    assert len(result.Logs) == 1

    # Wrong mode — skipped by super() mode gate
    ctx = MigrationContext(ExecutionMode="READONLY", Changes=[None])  # type: ignore[list-item]
    result = pipeline.run(ctx)
    assert len(result.Logs) == 0
