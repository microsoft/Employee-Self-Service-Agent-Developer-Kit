...
4. **Otherwise, a real three-way merge** of `base_payload` (from `ca-baseline.json`), `ours` (projected live), `theirs` (DA template):
   - `ours == base` **and** the enabled/disabled state matches the template → `UNCHANGED` (customer didn't touch it; nothing to carry).
...
**Enabled/disabled state (`_apply_state`).** A topic's `state`/`status` lives *beside* the payload, not inside it, so a structural payload merge never sees it. The DA template ships each topic with a state; if the customer set a different one — e.g. disabling the `ConversationStart` greeting the template itself suggests turning off — that is a real customization. `_merge_one` compares the customer's state (via `projection._state_of`) against the target's state and carries it onto the DA component, so a state-only change lands `MERGED` (not a false `UNCHANGED`) and a state change rides along with a payload merge. **State is tri-valued:** `_state_of` returns `"Active"`, `"Inactive"`, or **`None` when the source records no usable `statecode`**. An *unknown* state is never treated as a change — the tool keeps the template's state and claims nothing, so a topic the DA ships disabled is not silently re-enabled off missing metadata (and the report never says "you enabled this" without a source state that says so). The `ca-baseline.json` baseline does not record state, so a known change is compared against the DA target's shipped state and always surfaced in the result detail for verification.

**Action ordering (`_merge_list`).** A Power Fx action list is imperative, so order is load-bearing: an action inserted *before* a terminal action (e.g. `EndDialog`) must stay before it, and a **reorder is itself a customization**. `_merge_list` first reconciles each action's *value* by `id` (inserts, deletes, edits told apart), then merges the **order** three-way over the actions common to both sides: if only the customer reordered them, adopt the customer's order; if only ESS did, keep ESS's; if both reordered the same set differently, keep ESS's order and raise an ordering **conflict** (order can't be attributed without a baseline, so there it falls back to ESS's sequence). Onto that spine it weaves each surviving action the *other* side inserted, placed **after the nearest preceding action that still exists** (the anchor that encodes where it went). If no preceding action survives and the insert was not at the very front, its position cannot be preserved, so it raises a positioning **conflict** rather than silently appending it to the end.

**Complete customer version on conflict (`ComponentResult.customer_version`).** Keeping ESS's component whole is the safe default for a `CONFLICTED` component, but `conflicts` only names the *contested* nodes — any edit the customer made that merged cleanly is discarded with the dropped merge and would otherwise vanish from both package and report. So a `CONFLICTED` result also carries `describe(component)` — the customer's complete version — which the report reproduces, guaranteeing every unapplied edit is on record for manual re-application. The payload dump does **not** carry lifecycle state, so when a conflict also drops a known state change, `_record_unapplied_state` sets a structured `ComponentResult.customer_state` and a detail line (rendered in the report) so the disabled/enabled setting stays visible and actionable for hand re-application even though ESS's component was kept.
...
- **Graceful degradation:** if the model is unreachable, `LlmUnavailable` is raised (never `sys.exit`); the migration still produces a package, keeps the DA's instructions, and reports the edit as a conflict for hand re-application.
- **Offline paths are two distinct signals, not one.** `inspect` passes `keep_target_instructions`, a fast, network-free *prediction* that the edit can migrate (returns the DA text, reported optimistically) — it must stay optimistic because the real `migrate` uses the model by default. `migrate --keep-instructions`, by contrast, is a real run where the maker opted out of the model, so it passes `skip_instruction_reconciliation`, which raises `InstructionReconciliationSkipped`: `_reconcile_gpt_instructions` then keeps the DA wording and **leaves the `.instructions` conflict in place**, landing the component `CONFLICTED` with an honest note — never a `MERGED` claiming the model re-applied an edit it never saw. This matches the `--keep-instructions` help text ("report the instruction edit as a conflict instead").
