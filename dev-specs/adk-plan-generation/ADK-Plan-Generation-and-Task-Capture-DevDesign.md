...
- **`src/skills/planner/SKILL.md`** — the playbook, with sub‑files: `research.md` (the Learn crawl, §7), `interview.md` (§8), `model.md` (the Plan schema + writer rules, §9), `assign.md` (Flow 1, §10), `evaluate.md` (the theoretical scenario‑eval hand‑off, §10.2), `mytasks.md` (Flow 2 + the Learn‑enriched task brief, §11), `edit.md` (the Markdown round‑trip, §9.5), and `capture.md` (§12).
...
**Auto‑eval after authoring — a theoretical, scenario‑based eval (Phase 5).** Once the plan is authored (tasks emitted + assigned), the planner **automatically invokes the eval skill** (`src/skills/evaluations/create/SKILL.md`, the `/evaluate` skill) to generate a **theoretical** eval from the **scenarios captured in Context** (the `scenario` group) — before anything is built. This is a product design choice: turn the sponsor's chosen scenarios into acceptance tests up front, so *what "good" looks like* is visible pre‑build. **The planner does not own the eval skill** — it only invokes it and hands over the scenarios; it authors no eval content and reimplements nothing (§6, "orchestrates; never re‑implements"). *Interface (what the planner passes):* the scenario ids + display names **and the enabled scenarios per category (`scenarioCapability`, §9.4) — the topic‑level unit golden prompts are written for**, the sponsor's `jtbd`/`objective`/success measure, the target `system.*` per scenario (for integration‑data framing), and `persona`/`market`. *Mode:* theoretical/pre‑build → generate the scenario test sets into the local eval workspace and **stop before the Dataverse push** (there is no environment yet). *Relationship to the "Generate evals" Task:* this Phase‑5 eval is the **scenario‑driven seed**; the plan's **Generate evals** Task (`/evaluate`, `eval-author`, consumes `primaryEnvironment` + topics) is the **topic‑driven** run that refines and actually runs the suite against the built agent later — the task list is unchanged. *Graceful degradation:* if in a given build the eval skill strictly needs a built agent/topics and cannot run theoretically, the planner says so plainly and defers to the Generate‑evals Task; plan creation still completes. New sub‑file: `src/skills/planner/evaluate.md`.
