...
Reference samples for the Microsoft Employee Self-Service (ESS) agent built on Microsoft Copilot Studio. Each folder contains ready-to-use assets — Copilot Studio topic YAMLs, ESS Template Configuration XMLs, evaluation test sets, and supporting documentation — that customers can import directly into their environment to extend or customize their ESS agent.
...
| Folder | Description |
|--------|-------------|
| [Evaluations/](./Evaluations/) | Evaluation test sets and templates for measuring ESS agent quality (accuracy, safety, grounding, completeness) using Copilot Studio's Evaluation tool. Includes ready-to-use Starter test sets and customizable Templated test sets. |
...
4. Use the trigger queries inside each `topic.yaml` as seed prompts for validation, and the assets under [`Evaluations/`](./Evaluations/) for repeatable quality checks.
